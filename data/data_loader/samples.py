...
class SysuSamples:
    ...
    def _load_samples(self):
        ...
        print(
            "Note: pids of rgb_train_set and ir_train_set are {} equal".format(
                self._is_equal(self.rgb_samples_train, self.ir_samples_train, 1)
            )
        )
        print(
            "Note: pids of rgb_val_set and ir_val_set are {} equal".format(
                self._is_equal(self.rgb_samples_val, self.ir_samples_val, 1)
            )
        )

        self.rgb_samples_train = copy.deepcopy(
            self.rgb_samples_train + self.rgb_samples_val
        )
        self.ir_samples_train = copy.deepcopy(
            self.ir_samples_train + self.ir_samples_val
        )

        self.rgb_samples_all = copy.deepcopy(
            self.rgb_samples_train + self.rgb_samples_test
        )
        self.ir_samples_all = copy.deepcopy(
            self.ir_samples_train + self.ir_samples_test
        )
        self.rgb_ir_samples_train = copy.deepcopy(
            self.rgb_samples_train + self.ir_samples_train
        )
        if self.reorder:
            assert self._is_equal(
                self.rgb_samples_train, self.ir_samples_train, 1
            ), "reorder of rgb and ir separately will cause errors"
            self.rgb_ir_samples_train = self._reorder(self.rgb_ir_samples_train, 1)
        ...
        print("rgb_ir_samples_train", self._anaplsis_samples(self.rgb_ir_samples_train))
