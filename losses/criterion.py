...
class Criterion(object):
    ...
    VALUE_FACTORY = [
        ...
        "logits",
        "pids",
        "camids",  # for vanilla traning
        "feats_s",
        "logits_s",
        "feats_t",
        "logits_t",  # distillation
        "reduce",
    ]
    ...
    def __init__(self, criterion_list):
        ...
        for criterion in criterion_list:
            assert (
                criterion["criterion"].__class__.__name__ in Criterion.CRITERION_FACTORY
            ), "expect one of {}, but got {}".format(
                Criterion.CRITERION_FACTORY, criterion["criterion"].__class__.__name__
            )
    ...
    def compute(self, **kwargs):
        ...
        for arg in kwargs.keys():
            assert (
                arg in Criterion.VALUE_FACTORY
        ...
        for value in self.criterion_list:
            ...
            if criterion.__class__.__name__ in [
                "CrossEntropyLoss",
                "CrossEntropyLabelSmooth",
                "FocalLoss",
                "CenterLoss",
                ...
                if isinstance(
                    ...
                    for idx in range(len(kwargs["logits"])):
                        loss += weight * criterion(
                            kwargs["logits"][idx], kwargs["pids"]
                        )
                ...
                assert (
                    "head_feats" in kwargs.keys()
                ), "SimDistillLoss expect feats as inputs, but got {}".format(
                    kwargs.keys()
                )
                assert isinstance(
                    kwargs["head_feats"], list
                ), "SIMSelfDistillLoss expect head_feats is type list, but got type{}".format(
                    type(kwargs["head_feats"])
                )
                loss = weight * criterion(feats_list=kwargs["head_feats"])

            elif criterion.__class__.__name__ in ["ProbSelfDistillLoss"]:
                assert (
                    "logits" in kwargs.keys()
                ), "SimDistillLoss expect feats as inputs, but got {}".format(
                    kwargs.keys()
                )
                ...
                assert (
                    "logits_s" in kwargs.keys() and "logits_t" in kwargs.keys()
                ), "KLLoss expect logits_s and logits_t as inputs, but got {}".format(
                    kwargs.keys()
                )
                if isinstance(kwargs["logits"], list):
                    loss = weight * criterion(
                        logits_s=kwargs["logits_s"][0],
                        logits_t=kwargs["logits_t"].detach(),
                    )
                else:
                    loss = weight * criterion(
                        logits_s=kwargs["logits_s"],
                        logits_t=kwargs["logits_t"].detach(),
                    )

            else:
                assert 0, "expect criterion in {} but got {}".format(
                    Criterion.CRITERION_FACTORY, criterion
                )
